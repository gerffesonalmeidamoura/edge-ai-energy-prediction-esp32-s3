...
// ===== Config =====
#define MODEL_NAME  "TCN_MONO_COM_INTERHARMONICAS_INT8"
#define ARENA_SIZE  196608          // 192 KB (~182 KB sugeridos + folga)
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1

// I2C p/ INA226 (seus pinos)
#define I2C_SDA     8
#define I2C_SCL     9
#define INA226_ADDR 0x40
#define RSHUNT_OHM  0.1f

#include "model_data.h"  // const unsigned char tcn_model[]; const int tcn_model_len;
...
void setup() {
  ...
  // Modelo
  const tflite::Model* model = tflite::GetModel(tcn_model);
  ...
  // ===== Resolver para TCN INT8 (com SHAPE) =====
  static tflite::MicroMutableOpResolver<29> resolver;
  resolver.AddConv2D();
  resolver.AddDepthwiseConv2D();
  resolver.AddAdd();                 // residual/skip
  resolver.AddMul();
  resolver.AddAveragePool2D();
  resolver.AddShape();               // necessário neste modelo
  ...
  resolver.AddFullyConnected();      // cabeça densa (se houver)
  resolver.AddQuantize();
  resolver.AddDequantize();          // opcional — mantém por segurança
  resolver.AddSoftmax();             // se a saída usar

  static tflite::MicroInterpreter interpreter(model, resolver, tensor_arena, ARENA_SIZE);
  interp = &interpreter;

  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Tente ARENA_SIZE = 212992 (208 KB) ou 229376 (224 KB).");
    while (1) delay(1000);
  }
  ...
  if (input->type != kTfLiteInt8 || output->type != kTfLiteInt8) {
    Serial.println("ERRO: esperado I/O INT8 para este TCN.");
    while (1) delay(1000);
  }
  ...
  // Preenche entrada com “zero lógico” (zp = -65 conforme seu diagnóstico)
    ...
    case PH_RUN:
      if (iter < RUNS) {
        ...
        Serial.printf("CSV:%s,%d,%u,%d,%.2f,%llu,%llu,%.1f\n",
                      MODEL_NAME, tcn_model_len, (unsigned)ARENA_SIZE, RUNS,
        ...
        Serial.printf("CSV:mem,%s,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
                      MODEL_NAME, tcn_model_len, (unsigned)ARENA_SIZE,
