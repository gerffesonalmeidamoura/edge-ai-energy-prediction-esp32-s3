...
// ===== Config =====
#define MODEL_NAME  "MLP_MONO_COM_INTERHARMONICAS"
#define ARENA_SIZE  98304          // ≈96 KB
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
#include "model_data.h"  // const unsigned char mlp_model[]; const int mlp_model_len;
...
static const char* tname(TfLiteType t){
  switch(t){ case kTfLiteFloat32:return "float32"; case kTfLiteInt8:return "int8";
             case kTfLiteUInt8:return "uint8"; case kTfLiteInt16:return "int16"; default:return "?"; }
}
static void print_shape(const TfLiteTensor* t){
  if (!t || !t->dims) { Serial.print("[]"); return; }
  Serial.print("[");
  for (int i=0;i<t->dims->size;i++){ Serial.print(t->dims->data[i]); if (i+1<t->dims->size) Serial.print(", "); }
  Serial.print("]");
}
...
// Mede baseline médio rápido (idle)
...
void setup() {
  ...
  if (!ina.begin()) {
    Serial.println("INA226 nao encontrado. Confira I2C/endereco.");
    while (1) delay(1000);
  }
  ina.reset();
  ...
  // Arena em PSRAM (pré-preenche com watermark para estimar uso real)
  tensor_arena = (uint8_t*) heap_caps_malloc(ARENA_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!tensor_arena) {
    Serial.printf("Falha ao alocar %u bytes na PSRAM.\n", (unsigned)ARENA_SIZE);
    while (1) delay(1000);
  }
  memset(tensor_arena, 0xA5, ARENA_SIZE);  // watermark
  ...
  arena_psram_bytes      = (free_psram_before > free_psram_after_arena) ?
                           (free_psram_before - free_psram_after_arena) : 0;
  ...
  // Modelo (usa mlp_model / mlp_model_len)
  const tflite::Model* model = tflite::GetModel(mlp_model);
  ...
  // Resolver mínimo para MLP INT8
  static tflite::MicroMutableOpResolver<28> resolver;
  ...
  if (input->type != kTfLiteInt8 || output->type != kTfLiteInt8) {
    Serial.println("ERRO: modelo não é INT8 em I/O.");
    while (1) delay(1000);
  }
  ...
  // Estimar uso real da arena (watermark): varre de trás pra frente até achar != 0xA5
  ...
  // Preenche entrada com zero lógico (zp) — usa zp do próprio tensor (para este modelo, -48)
  ...
  switch (phase) {
    ...
    case PH_RUN:
      if (iter < RUNS) {
        ...
        if (st != kTfLiteOk) {
          Serial.printf("Invoke falhou em r=%d\n", iter);
          phase = PH_DONE; break;
        }
        ...
        // Linha-resumo de desempenho (para planilha)
        ...
        Serial.printf("CSV:%s,%d,%u,%d,%.2f,%llu,%llu,%.1f\n",
                      MODEL_NAME, mlp_model_len, (unsigned)ARENA_SIZE, RUNS,
                      mean_us, (unsigned long long)best_us, (unsigned long long)worst_us, baseline_mW);

        // Linha-resumo de memória (para planilha)
        ...
        Serial.printf("CSV:mem,%s,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
                      MODEL_NAME, mlp_model_len, (unsigned)ARENA_SIZE,
                      (unsigned)heap_caps_get_free_size(MALLOC_CAP_INTERNAL - 0 + 0), // já capturado antes, mas imprimimos valores finais também
