...
// ===== Config =====
#define MODEL_NAME  "ESN_MONO_SEM_HARMONICAS_INT8"
#define ARENA_SIZE  69632            // ~68 KB (≈ 67.92 KB sugeridos)
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
#include "model_data.h"  // define: g_model[], g_model_len
...
void setup() {
  ...
  // ===== Resolver mínimo p/ ESN (FC INT8) =====
  static tflite::MicroMutableOpResolver<8> resolver;
  resolver.AddFullyConnected();
  resolver.AddReshape();
  resolver.AddQuantize();
  resolver.AddDequantize();   // por segurança
  resolver.AddAdd();          // segurança (se houver bias separado)
  resolver.AddMul();          // segurança
  resolver.AddLogistic();     // se saída for sigmoid (nem sempre necessário)
  resolver.AddTanh();         // caso a saída seja tanh
  ...
  if (input->type != kTfLiteInt8 || output->type != kTfLiteInt8) {
    Serial.println("ERRO: esperado I/O INT8 para este ESN.");
    while (1) delay(1000);
  }
  ...
  Serial.printf(" | type=%s | bytes=%d | q=(scale=%.8g, zp=%d)\n",
  ...
  // Preenche a entrada com o zero_point do modelo (zp = -1 do diagnóstico)
