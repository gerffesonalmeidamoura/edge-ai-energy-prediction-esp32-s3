...
// ===== Config =====
#define MODEL_NAME  "ESN_MONO_FP16"
#define ARENA_SIZE  69632           // ~68 KB (≈ 67.9 KB sugeridos + folga)
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
void setup() {
  ...
  // ===== Resolver: ESN FP16 (tipicamente apenas FC; inclui Dequantize por segurança p/ FP16→FP32) =====
  static tflite::MicroMutableOpResolver<6> resolver;
  resolver.AddFullyConnected();
  resolver.AddReshape();
  resolver.AddLogistic();     // se a saída tiver ativação sigmoid; não atrapalha se não existir
  resolver.AddQuantize();     // harmless se não usado
  resolver.AddDequantize();   // útil para modelos FP16 que internamente convertem para FP32
  resolver.AddMul();          // opcional (caso tenha normalização simples no grafo)

  static tflite::MicroInterpreter interpreter(model, resolver, tensor_arena, ARENA_SIZE);
  interp = &interpreter;

  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Tente ARENA_SIZE = 73728 (72 KB) ou 81920 (80 KB).");
    while (1) delay(1000);
  }
  ...
  if (input->type != kTfLiteFloat16 || output->type != kTfLiteFloat16) {
    Serial.printf("AVISO: Esperado I/O FLOAT16, mas recebi in=%s out=%s\n", tname(input->type), tname(output->type));
  }
  ...
  // Preenche a entrada com zeros (FP16 -> bits 0x0000)
