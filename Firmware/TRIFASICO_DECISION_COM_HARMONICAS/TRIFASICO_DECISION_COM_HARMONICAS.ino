...
// ===== Config =====
#define MODEL_NAME   "DT_PCA_TRI_FP32"
#define WARMUP       5
#define RUNS         60
#define PIN_PROBE    -1

// Opcional: tamanho do modelo em Flash (bytes) só para o CSV.
// Coloque aqui o valor real se quiser registrar no relatório.
#ifndef MODEL_BYTES
  #define MODEL_BYTES  0   // ex.: 26050 (~26.05 KB)
#endif
...
// Header gerado para a árvore (contém dt_predict e DT_INPUT_DIM=50)
#include "model_data.h"

// A função pública do header gerado:
extern "C" float dt_predict(const float f[DT_INPUT_DIM]);
...
void setup() {
  ...
  // (Sem arena/TFLM) – apenas fotografamos depois
...
void loop() {
  static float feats[DT_INPUT_DIM];   // 50 componentes PCA
  switch (phase) {
    case PH_WARMUP:
      if (iter < WARMUP) {
