...
// ===== Config =====
#define MODEL_NAME  "CNND1_MONO_COM_HARMONICAS_INT8"
#define ARENA_SIZE  196608          // 192 KB (~182 KB sugeridos + folga). Se faltar, tente 212992 ou 229376.
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
#include "model_data.h"  // deve definir: alignas(16) const unsigned char g_model[]; const int g_model_len;
...
void setup() {
  ...
  // ===== Resolver para CNND1 INT8 (com SHAPE/STRIDED_SLICE/PACK) =====
  ...
  resolver.AddDequantize();      // por segurança (full-int normalmente não precisa)
  resolver.AddShape();           // requerido
  resolver.AddStridedSlice();    // requerido
  resolver.AddPack();            // requerido
  // Opcional manter ReLU caso não esteja fundido
  resolver.AddRelu();
  resolver.AddRelu6();
  ...
  // Preenche entrada com zero lógico (zp ≈ -105 do diagnóstico deste modelo)
