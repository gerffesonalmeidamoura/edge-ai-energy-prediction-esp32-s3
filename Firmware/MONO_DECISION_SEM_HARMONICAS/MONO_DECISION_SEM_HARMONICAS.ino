...
// ===== Config =====
#define MODEL_NAME   "DT_PCA_MONO_SEM_HARMONICAS_FP32"
#define WARMUP       5
#define RUNS         60
#define PIN_PROBE    -1

// Opcional: tamanho do modelo em bytes (só para o CSV). Ajuste se quiser ver no log.
// Ex.: 24.02 KB ≈ 24020 bytes
#ifndef MODEL_BYTES
  #define MODEL_BYTES  24020
#endif
...
// Header gerado: expõe `DT_INPUT_DIM` e a função inline `dt_predict(...)`
...
void setup() {
  ...
  // (Sem arena/TFLM: só observamos estado pós-setup)
...
void loop() {
  static float feats[DT_INPUT_DIM];   // vetor PCA (dimensão do header)
  switch (phase) {
    case PH_WARMUP:
      if (iter < WARMUP) {
        for (int i=0;i<DT_INPUT_DIM;i++) feats[i] = 0.0f; // padrão fixo
