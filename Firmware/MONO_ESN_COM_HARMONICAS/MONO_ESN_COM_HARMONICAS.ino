...
// ===== Config =====
#define MODEL_NAME  "ESN_MONO_INT8"
#define ARENA_SIZE  65536     // 64 KB (sobra para este modelo)
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
#include "model_data.h" // const unsigned char g_model[]; const int g_model_len;
...
void setup() {
  ...
  if (!ina.begin()) { Serial.println("INA226 nao encontrado."); while (1) delay(1000); }
  ina.reset();
  ...
  if (!tensor_arena) { Serial.println("Falha arena."); while (1) delay(1000); }
  ...
  // Resolver mínimo p/ ESN (Dense INT8)
  static tflite::MicroMutableOpResolver<8> resolver;
  resolver.AddFullyConnected();
  resolver.AddReshape();
  resolver.AddQuantize();
  resolver.AddDequantize();
  // extras defensivos (se o conversor inseriu algo simples)
  resolver.AddAdd();
  resolver.AddMul();
  ...
  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Tente ARENA_SIZE maior (ex.: 98304).");
    while (1) delay(1000);
  }
  ...
  Serial.printf(" | type=%s | bytes=%d", tname(input->type), input->bytes);
  if (input->type==kTfLiteInt8) Serial.printf(" | q=(scale=%.6g, zp=%d)", input->params.scale, input->params.zero_point);
  Serial.println();
  Serial.print("Output: shape="); print_shape(output);
  Serial.printf(" | type=%s | bytes=%d", tname(output->type), output->bytes);
  if (output->type==kTfLiteInt8) Serial.printf(" | q=(scale=%.6g, zp=%d)", output->params.scale, output->params.zero_point);
  Serial.println();

  // Pré-preenche entrada com “zero lógico”
  ...
  switch (phase) {
    ...
    case PH_RUN:
      if (iter < RUNS) {
        ...
        float avgP_net  = avgP_mW - baseline_mW;
        ...
        Serial.printf("CSV:%s,%d,%u,%d,%.2f,%llu,%llu,%.1f\n",
                      MODEL_NAME, (int)g_model_len, (unsigned)ARENA_SIZE, RUNS,
        ...
        Serial.printf("CSV:mem,%s,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
                      MODEL_NAME, (int)g_model_len, (unsigned)ARENA_SIZE,
