// ESN_TRI_COM_INTERHARMONICAS_FP32.ino
#include <Arduino.h>
#include <WiFi.h>
#include "esp_timer.h"
...
// ===== Config =====
#define MODEL_NAME   "ESN_TRI_COM_INTERHARMONICAS_FP32"
#define ARENA_SIZE   73728        // 72 KB  (~69.9 KB sugeridos + folga). Se faltar, tente 81920 (80 KB).
#define WARMUP       5
#define RUNS         60
#define PIN_PROBE    -1           // opcional para medir em GPIO o intervalo de inferência

// (Opcional) Informe o tamanho exato do .tflite apenas para o CSV final.
#ifndef MODEL_BYTES
  #define MODEL_BYTES  (2080)     // ≈ 2.03 KB -> ajuste se quiser o valor exato
#endif

// I2C do INA226 (ajuste aos seus pinos)
#define I2C_SDA      8
#define I2C_SCL      9
#define INA226_ADDR  0x40
#define RSHUNT_OHM   0.1f

// ===== Modelo (gerado por GERAR_H.py) =====
// Espera: alignas(16) const unsigned char g_model[]; const int g_model_len;
...
void setup() {
  ...
  disable_radios();
  esp_task_wdt_deinit();
  disableLoopWDT(); disableCore0WDT(); disableCore1WDT();
  ...
  // ===== Resolver para ESN FP32: apenas FC (e, por segurança, Reshape/Quantize/Dequantize) =====
  static tflite::MicroMutableOpResolver<8> resolver;
  resolver.AddFullyConnected();
  resolver.AddReshape();
  resolver.AddQuantize();     // não deve ser usado em FP32, mas é inofensivo se o modelo trouxer
  resolver.AddDequantize();   // idem
  resolver.AddAdd();          // defensivo (às vezes bias é Add separado)
  resolver.AddMul();          // defensivo
  resolver.AddLogistic();     // defensivo (caso exporte Sigmoid)
  resolver.AddRelu();         // defensivo

  static tflite::MicroInterpreter interpreter(model, resolver, tensor_arena, ARENA_SIZE);
  interp = &interpreter;

  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Tente ARENA_SIZE = 81920 (80 KB) ou 90112 (88 KB).");
    while (1) delay(1000);
  }
  ...
  Serial.printf(" | type=%s | bytes=%d\n", tname(input->type), input->bytes);
  Serial.print("Output: shape="); print_shape(output);
  Serial.printf(" | type=%s | bytes=%d\n", tname(output->type), output->bytes);

  // Preenche entrada com zeros (features do reservatório já “prontas”: [1,500] float32)
  if (input->type == kTfLiteFloat32) {
    float* fin = input->data.f;
    for (int i=0; i<input->bytes/sizeof(float); ++i) fin[i] = 0.0f;
  }
...
void loop() {
  ...
  switch (phase) {
    case PH_WARMUP:
      if (iter < WARMUP) {
        // Entrada dummy zerada a cada ciclo (estabiliza caches/leitura)
        TfLiteTensor* input = interp->input(0);
        if (input->type == kTfLiteFloat32) {
          float* fin = input->data.f;
          for (int i=0; i<input->bytes/sizeof(float); ++i) fin[i] = 0.0f;
