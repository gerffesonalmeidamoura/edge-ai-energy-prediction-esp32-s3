...
// ===== Config =====
#define MODEL_NAME  "MLP_TRI_COM_INTERHARMONICAS"
#define ARENA_SIZE  98304          // ≈96 KB (baseado na arena sugerida ~95 KB)
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
void setup() {
  ...
  // Arena em PSRAM (pré-preenche com watermark p/ estimar uso real)
  ...
  // ===== Resolver (inclui QUANTIZE/DEQUANTIZE por segurança) =====
  ...
  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Considere aumentar ARENA_SIZE (ex.: 110592 ou 122880).");
    while (1) delay(1000);
  }
  ...
  // Esperado: FLOAT32 I/O (o grafo interno pode quantizar/desquantizar)
  if (input->type != kTfLiteFloat32 || output->type != kTfLiteFloat32) {
    Serial.println("ERRO: modelo esperado com I/O FLOAT32.");
    while (1) delay(1000);
  }
  ...
  // Uso real da arena (watermark): varre de trás pra frente até achar != 0xA5
  ...
  // Preenche entrada com zeros (float32)
  {
    const int n = input->bytes / sizeof(float);
