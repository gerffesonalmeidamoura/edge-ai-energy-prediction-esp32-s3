...
// ===== Config =====
#define MODEL_NAME  "CNN1D_TRI_COM_HARMONICAS_INT8"
...
#include "model_data.h"  // esperado: alignas(16) const unsigned char g_model[]; const int g_model_len;
...
void setup() {
  ...
  // ===== Resolver para CNN1D INT8 (inclui SHAPE) =====
  static tflite::MicroMutableOpResolver<32> resolver;
  resolver.AddShape();               // requerido pelo grafo
  ...
  // Extras defensivos — caso o modelo inclua em versões futuras:
  resolver.AddPad();
  ...
  // Preenche entrada com zero lógico (zp ≈ -75 segundo seu diagnóstico)
