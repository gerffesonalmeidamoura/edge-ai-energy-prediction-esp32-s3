...
// ===== Config =====
#define MODEL_NAME   "DT_PCA_FP32"   // renomeie se quiser identificar (ex.: DT_PCA_TRI_COM_HARMONICAS_FP32)
#define WARMUP       5
#define RUNS         60
#define PIN_PROBE    -1

// Tamanho do modelo em Flash (aprox. 24.61 KB)
#ifndef MODEL_BYTES
  #define MODEL_BYTES  24610
#endif
...
// Header da árvore (header-only, com PCA->DT em FP32)
// Deve expor:  #define DT_INPUT_DIM 50   e   static inline float dt_predict(const float f[DT_INPUT_DIM]);
#include "model_data.h"

// Confirme que a função pública é dt_predict (igual ao gerado pelo script)
...
#else
  Serial.println("Input:  shape=[1,50] | type=float32 | PCA features");
#endif
...
void loop() {
#ifdef DT_INPUT_DIM
  static float feats[DT_INPUT_DIM];
#else
  static float feats[50];
#endif
...
#else
        for (int i=0;i<50;i++) feats[i] = 0.0f;
#endif
    ...
    case PH_RUN:
      if (iter < RUNS) {
        ...
        Serial.printf("CSV:mem,%s,%d,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u\n",
                      ...
                      (unsigned)free_int_before, (unsigned)free_psram_before,         // sem arena
