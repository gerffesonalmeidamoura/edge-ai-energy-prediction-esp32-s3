...
// ===== Config =====
#define MODEL_NAME  "MLP_MONO_SEM_HARMONICAS"
#define ARENA_SIZE  98304          // ≈96 KB (base ~95 KB)
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
void setup() {
  ...
  if (model->version() != TFLITE_SCHEMA_VERSION) { Serial.printf("Schema mismatch: model %d vs TFLite %d\n", model->version(), TFLITE_SCHEMA_VERSION); while (1) delay(1000); }

  dump_ops(model);

  // ===== Resolver (inclui QUANTIZE/DEQUANTIZE) =====
  static tflite::MicroMutableOpResolver<28> resolver;
  resolver.AddFullyConnected();
  resolver.AddQuantize();
  resolver.AddDequantize();
  // extras seguros (não atrapalham se não usados)
  resolver.AddRelu();
