// ESN_TRI_SEM_HARMONICAS_READOUT_FP32.ino
#include <Arduino.h>
#include <WiFi.h>
#include "esp_timer.h"
...
// ===== Config =====
#define MODEL_NAME   "ESN_TRI_SEM_HARMONICAS_READOUT_FP32"
#define WARMUP       5
#define RUNS         60
#define PIN_PROBE    -1

// Estimativa de "tamanho do modelo" (pesos float32 + bias) só para o CSV
// (= ESN_DIM*4 + 4). Se preferir, pode fixar um número.
#define MODEL_BYTES  (ESN_DIM * 4 + 4)

// I2C do INA226 (ajuste se necessário)
#define I2C_SDA      8
#define I2C_SCL      9
#define INA226_ADDR  0x40
#define RSHUNT_OHM   0.1f

// Header com os pesos do readout: ESN_DIM, ESN_W[], ESN_B e esn_predict()
...
void setup() {
  ...
  // (Sem arena/TFLM; apenas confirmamos depois)
  ...
  Serial.printf("Input:  shape=[1,%d] | type=float32 | reservoir states\n", (int)ESN_DIM);
...
void loop() {
  static float feats[ESN_DIM];   // vetor de estados do reservatório (dummy aqui)
  switch (phase) {
    case PH_WARMUP:
      if (iter < WARMUP) {
        for (int i=0;i<ESN_DIM;i++) feats[i] = 0.0f;   // insira seus estados reais aqui, se quiser
        (void)esn_predict(feats);
        iter++; delay(0);
      } else {
    ...
    case PH_RUN:
      if (iter < RUNS) {
        ...
        for (int i=0;i<ESN_DIM;i++) feats[i] = 0.0f;   // substitua por estados reais se desejar medir o caso real

        float p0_mW = read_power_mW_calc();
        uint64_t t0 = now_us();

        volatile float y = esn_predict(feats);
