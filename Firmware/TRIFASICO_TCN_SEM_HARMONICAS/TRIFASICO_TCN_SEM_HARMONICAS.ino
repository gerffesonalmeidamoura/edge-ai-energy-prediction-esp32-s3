// Arquivo .ino para TCN_TRI_SEM_HARMONICAS_INT8
// Baseado no padrão fornecido
...
// ===== Config =====
#define MODEL_NAME  "TCN_TRI_SEM_HARMONICAS_INT8"
#define ARENA_SIZE  186368          // 182.12 KB sugerido + ~4 KB folga
#define WARMUP      5
#define RUNS        60
#define PIN_PROBE   -1
...
void setup() {
  ...
  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Tente ARENA_SIZE maior.");
    while (1) delay(1000);
  }
  ...
  arena_headroom_bytes = ARENA_SIZE - arena_used_bytes_est;
  ...
  // Preenche input com valor lógico neutro (zp = 20)
