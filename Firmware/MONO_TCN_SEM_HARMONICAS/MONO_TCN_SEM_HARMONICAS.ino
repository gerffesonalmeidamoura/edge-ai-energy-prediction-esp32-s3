...
// ===== Config =====
#define MODEL_NAME  "TCN_MONO_SEM_HARMONICAS_INT8"
...
void setup() {
  ...
  // ===== Resolver para TCN INT8 (essencial + SHAPE) =====
  ...
  // opcional (mantidos por segurança, caso existam no grafo):
  ...
  if (kTfLiteOk != interp->AllocateTensors()) {
    Serial.println("AllocateTensors FAILED. Tente ARENA_SIZE = 106496 (104 KB) ou 122880 (120 KB).");
    while (1) delay(1000);
  }
  ...
  // Preenche entrada com zero lógico (zp = -93 no diagnóstico)
