...
// ===== Config =====
#define MODEL_NAME  "CNND1_MONO_COM_INTERHARMONICAS_INT8"
...
#include "model_data.h"  // alignas(16) const unsigned char g_model[]; const int g_model_len;
...
void setup() {
  ...
  // ===== Resolver para CNND1 INT8 (inclui SHAPE/STRIDED_SLICE) =====
  static tflite::MicroMutableOpResolver<32> resolver;
  resolver.AddDepthwiseConv2D();
  resolver.AddConv2D();
  resolver.AddAveragePool2D();

  resolver.AddShape();               // necessário
  resolver.AddStridedSlice();        // necessário
  resolver.AddPack();
  resolver.AddReshape();
  ...
  resolver.AddQuantize();            // seguro manter
  resolver.AddDequantize();          // seguro manter
  // Outras ops comuns (não usadas aqui, mas não fazem mal):
  // resolver.AddAdd(); resolver.AddMul(); resolver.AddConcatenation(); resolver.AddPad(); resolver.AddUnpack();
  ...
  // Preenche entrada com zero lógico (zp do input)
